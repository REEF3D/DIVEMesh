...
void resize_class::Iarray(int ***& field, int numi, int numj, int **numk)
{
	...
	for(n=0; n<numi; ++n)
	for(m=0; m<numj; ++m)
	field[n][m] = new int[numk[n][m]];

	for(n=0; n<numi;++n)
	for(m=0;m<numj;++m)
	for(q=0;q<numk[n][m];++q)
	field[n][m][q]=0;
}

void resize_class::carray(char *& field, int numi)
{	
	if(numi>0)
	field = new char[numi];
}

void resize_class::carray(char **& field, int numi, int numj)
{
	int i;
...
void resize_class::carray(char ***& field, int numi, int numj, int numk)
{
