...
void lexer::gridspacing()
{
    ...
    {
        bool xyz = knoy>1; // true if 3D, false if 2D

        DXM = dx = (1.0/(2.0+double(xyz))) * ((xmax-xmin)/double(knox) + (xyz?((ymax-ymin)/double(knoy)):0) + (zmax-zmin)/double(knoz));

        DXYM = (1.0/(1.0+double(xyz))) * ((xmax-xmin)/double(knox) + (xyz?((ymax-ymin)/double(knoy)):0));
    }
