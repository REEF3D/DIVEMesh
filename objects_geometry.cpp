...
objects_geometry::objects_geometry()
{
}
...
void objects_geometry::start(lexer* p, dive* a)
{
	a->vertice_num=0;
	a->polygon_num=0;
	a->polygon_sum=0;
	...
	for(qn=0;qn<p->O61;++qn)
	wedge(p,a,qn);
	
	for(qn=0;qn<p->O43;++qn)
	conical(p,a,qn);
	
	for(qn=0;qn<p->O32;++qn)
	cylinder_y(p,a,qn);
...
void objects_geometry::box(lexer* p, dive* a,int rank)
{	
...
void objects_geometry::wedge(lexer* p, dive* a,int rank)
{	
...
// Polygon
	...
	a->polygon[a->polygon_num][0] = 3 + vertice_start;
	a->polygon[a->polygon_num][1] = 4 + vertice_start;
	a->polygon[a->polygon_num][2] = 5 + vertice_start;
	a->numvert[a->polygon_num] = 3;
	++a->polygon_num;
...
void objects_geometry::cylinder_y(lexer* p, dive* a,int rank)
{	
...
// Vertices	
	...
	cout<<"snum: "<<snum<<" U: "<<U<<" rank: "<<rank<<endl;
	
	//bottom circle
	for(n=0;n<snum;++n)
	{
	a->vertice[a->vertice_num][0] = xm + r1*cos(phi);
	a->vertice[a->vertice_num][1] = y1;
	a->vertice[a->vertice_num][2] = zm + r1*sin(phi);
	phi+=ds;
	++a->vertice_num;
	}
	...
	a->vertice[a->vertice_num][1] = y2;
	a->vertice[a->vertice_num][2] = zm + r2*sin(phi);
	phi+=ds;
	++a->vertice_num;
	}
...
void objects_geometry::cylinder_z(lexer* p, dive* a,int rank)
{	
...
void objects_geometry::conical(lexer* p, dive* a, int rank)
{	
	double U,ds,phi;
	double xm,ym,z1,z2,r1,r2;
	double rmax;
