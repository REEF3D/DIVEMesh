...
void solid::jacket_member_x(lexer *p, dive *a, int rank, int &ts, int &te )
{
    double U,ds,phi;
...
// Vertices	
	...
	p->tri_z[p->tricount][1] = zm1 + r1*cos(phi);
	
	p->tri_x[p->tricount][2] = xm1;
	p->tri_y[p->tricount][2] = ym1 + r1*sin(phi+ds);
	p->tri_z[p->tricount][2] = zm1 + r1*cos(phi+ds);
	++p->tricount;
	...
	p->tri_z[p->tricount][1] = zm2 + r2*cos(phi);
	
	p->tri_x[p->tricount][2] = xm2;
	p->tri_y[p->tricount][2] = ym2 + r2*sin(phi+ds);
	p->tri_z[p->tricount][2] = zm2 + r2*cos(phi+ds);
	++p->tricount;
	...
	p->tri_z[p->tricount][0] = zm1 + r1*cos(phi);
	
	p->tri_x[p->tricount][1] = xm2;
	p->tri_y[p->tricount][1] = ym2 + r2*sin(phi+ds);
	p->tri_z[p->tricount][1] = zm2 + r2*cos(phi+ds);
	...
	p->tri_z[p->tricount][2] = zm2 + r2*cos(phi);
