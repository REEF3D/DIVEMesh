...
void solid::jacket_member_y(lexer *p, dive *a, int rank, int &ts, int &te )
{
    double U,ds,phi;
...
// Vertices	
	...
	p->tri_y[p->tricount][1] = ym1;
	p->tri_z[p->tricount][1] = zm1 + r1*sin(phi);
	
	p->tri_x[p->tricount][2] = xm1 + r1*cos(phi+ds);
	p->tri_y[p->tricount][2] = ym1;
	p->tri_z[p->tricount][2] = zm1 + r1*sin(phi+ds);
	++p->tricount;
	...
	p->tri_y[p->tricount][1] = ym2;
	p->tri_z[p->tricount][1] = zm2 + r2*sin(phi);
	
	p->tri_x[p->tricount][2] = xm2 + r2*cos(phi+ds);
	p->tri_y[p->tricount][2] = ym2;
	p->tri_z[p->tricount][2] = zm2 + r2*sin(phi+ds);
	++p->tricount;
	...
	p->tri_y[p->tricount][0] = ym1;
	p->tri_z[p->tricount][0] = zm1 + r1*sin(phi);
	
	p->tri_x[p->tricount][1] = xm2 + r2*cos(phi+ds);
	p->tri_y[p->tricount][1] = ym2;
	p->tri_z[p->tricount][1] = zm2 + r2*sin(phi+ds);
	...
	p->tri_y[p->tricount][2] = ym2;
	p->tri_z[p->tricount][2] = zm2 + r2*sin(phi);
